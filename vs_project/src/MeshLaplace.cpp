...
void faceAngle(const Vec3d* points, const MeshTopo& topo, std::vector<Vec3d>& f2a)
{
	...
	for (int i = 0; i < f2a.size(); ++i)
	{
		e0 = points[topo.f2v[i][1]] - points[topo.f2v[i][0]];		e0.normalize_cond();
		e1 = points[topo.f2v[i][2]] - points[topo.f2v[i][1]];		e1.normalize_cond();
		e2 = points[topo.f2v[i][0]] - points[topo.f2v[i][2]];		e2.normalize_cond();
...
void laplaceTriplet(const OpenMesh::Vec3d* points, const MeshTopo& topo, std::vector<Eigen::Triplet<double>>& trips)
{
	...
	for (int i = 0; i < f2a.size(); ++i)
	{
		...
		/*f2cot[i][0] = 1.0 / tan(f2a[i][0]);
		f2cot[i][1] = 1.0 / tan(f2a[i][1]);
		f2cot[i][2] = 1.0 / tan(f2a[i][2]);*/
	}
